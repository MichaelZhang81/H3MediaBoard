...
# ★ 优化请求总超时（秒）：本地大模型（如 27B GGUF 生成 4096 token）可能耗时数分钟，
# 默认 660 秒。前端 web/prompt_optimizer_ui.js 的 MB_OPT_TIMEOUT_MS 与之配套
# （= 本值 + 5 秒裕量，让后端超时错误能回显到界面）。改这里 + 前端常量即可全局生效。
OPTIMIZE_TIMEOUT = 660
...
OPTIMIZER_KEYS_PATH = str(Path(__file__).resolve().parent / "optimizer_keys.json")


def load_optimizer_keys() -> dict:
    """本机 api key 存储（前端与工作流永不接触明文）。坏 JSON/缺失 -> {}。"""
    try:
        with open(OPTIMIZER_KEYS_PATH, "r", encoding="utf-8") as fh:
            data = json.load(fh)
...
def save_optimizer_keys(data: dict) -> bool:
    """写本机 keys.json；失败返回 False（调用方降级提示）。"""
    try:
        with open(OPTIMIZER_KEYS_PATH, "w", encoding="utf-8") as fh:
...
def get_optimizer_key(provider: str, api_url: str = "") -> str:
    """读取本机某服务商的 key：预设商按 provider 一一对应；自定义按 API URL 对应。
    未配置/文件损坏返回空串。"""
    keys = load_optimizer_keys()
    provider = str(provider or "")
    if provider == "custom":
        custom_keys = keys.get("custom_api_keys")
        return str((custom_keys or {}).get(str(api_url or ""), "")) if isinstance(custom_keys, dict) else ""
    api_keys = keys.get("api_keys")
    return str(api_keys.get(provider, "")) if isinstance(api_keys, dict) else ""


def set_optimizer_key(provider: str, api_url: str, value: str) -> bool:
    """写入本机某服务商的 key；空串即删除该条目。返回是否写盘成功。"""
    keys = load_optimizer_keys()
    provider = str(provider or "")
    value = str(value or "")
    if provider == "custom":
        url = str(api_url or "")
        custom_keys = keys.get("custom_api_keys") if isinstance(keys.get("custom_api_keys"), dict) else {}
        if value:
            custom_keys[url] = value
        else:
            custom_keys.pop(url, None)
        keys["custom_api_keys"] = custom_keys
    else:
        api_keys = keys.get("api_keys") if isinstance(keys.get("api_keys"), dict) else {}
        if value:
            api_keys[provider] = value
        else:
            api_keys.pop(provider, None)
        keys["api_keys"] = api_keys
    return save_optimizer_keys(keys)


def _inject_local_keys(config: dict) -> dict:
    """api 模式：从本机 keys.json 补 api_key（后端内部用，不外传）。
    预设商按 provider、自定义按 api_url 一一对应；未配置时为空串。"""
    if config.get("mode") != "local":
        config["api_key"] = get_optimizer_key(
            str(config.get("provider") or ""), str(config.get("api_url") or "")
        )
    return config


def _public_config(config: dict) -> dict:
    """脱敏配置：剔除 api_key/api_keys 明文，仅以 has_api_key 表明本机是否已配置。
    has_api_key 对空串做过滤：config/api_key 与本机 keys.json 中任一非空 key 才算已配置。"""
    keys = load_optimizer_keys()
    stored_api_keys = keys.get("api_keys") if isinstance(keys.get("api_keys"), dict) else {}
    custom_keys = keys.get("custom_api_keys") if isinstance(keys.get("custom_api_keys"), dict) else {}
    has_key = bool(config.get("api_key")) or bool(keys.get("api_key")) or any(
        value for value in stored_api_keys.values() if value
    ) or any(value for value in custom_keys.values() if value)
    public = {
        key: value for key, value in config.items()
        if key not in {"api_key", "api_keys"}
    }
    public["has_api_key"] = has_key
    public["runninghub_models"] = list(_RUNNINGHUB_MODELS_CACHE)
    public["runninghub_overseas_models"] = list(_RUNNINGHUB_OVERSEAS_MODELS_CACHE)
    return public
...
def _local_generate_impl(config: dict, payload: dict) -> str:
    ...
    try:
        ...
        if device == "cpu":
            model.to("cpu")
        # 模型加载可能耗时数秒：加载完成后、任何 generate 调用前补一次取消检查
        # （对齐 GGUF 路径同款），避免加载期间点取消要等整轮推理才生效。
        cancel_event = payload.get("_cancel_event")
...
def _gguf_n_gpu_layers(device: str) -> int:
    """local_device -> llama.cpp n_gpu_layers：cuda=-1（全量进显存）；cpu=0；auto 按 CUDA 可用性。"""
    import torch
    if device == "cpu":
        return 0
    if device == "cuda":
...
def _gguf_generate(config: dict, payload: dict) -> str:
    ...
    cancel_event = payload.get("_cancel_event")
    # 缓存键包含 local_device：换设备需卸载旧实例重建，否则 n_gpu_layers 变更不生效
    target_config = (str(model_path), str(mmproj_path), handler_name, str(config.get("local_device") or "auto"))
    try:
        ...
        with _GGUF_LOCK:
            if _GGUF_MODEL is None or _GGUF_CONFIG != target_config:
                ...
                _GGUF_MODEL = Llama(
                    ...
                    n_gpu_layers=_gguf_n_gpu_layers(str(config.get("local_device") or "auto")),
                    n_ctx=16384, verbose=False,
                )
...
async def _request_runninghub(config: dict, payload: dict) -> str:
    ...
    timeout = aiohttp.ClientTimeout(total=OPTIMIZE_TIMEOUT)
...
def _mask_api_key(text: str) -> str:
    """错误文本脱敏：gemini 的 api_key 拼在 URL 里，连接异常等会把完整 URL 带进
    异常文本；对外回显前把 key=xxx 打码，防止本机 API Key 泄漏到前端。"""
...
async def _request_async(config: dict, payload: dict) -> str:
    if config.get("mode") == "local":
        ...
        try:
            return await asyncio.wait_for(asyncio.to_thread(_local_generate, config, payload), timeout=OPTIMIZE_TIMEOUT)
        except asyncio.TimeoutError:
            cancel_event = payload.get("_cancel_event")
            if cancel_event:
                cancel_event.set()
            raise RuntimeError(f"提示词优化超时（{OPTIMIZE_TIMEOUT} 秒），已中断本地推理")
        except asyncio.CancelledError:
            cancel_event = payload.get("_cancel_event")
            if cancel_event:
                cancel_event.set()
    ...
    try:
        ...
        async with aiohttp.ClientSession(timeout=timeout) as session:
            async with session.post(url, headers=headers, json=body) as response:
                ...
                if response.status >= 400:
                    raise RuntimeError(f"API request failed ({response.status}): {_mask_api_key(text[:1000])}")
                data = json.loads(text)
    except asyncio.CancelledError:
        raise
    except (aiohttp.ClientError, asyncio.TimeoutError, ValueError) as error:
        raise RuntimeError(f"API request failed: {_mask_api_key(error)}") from error
    if config["protocol"] == "gemini":
        try:
...
async def get_prompt_optimizer_keys(request):
    """读取本机某服务商的 key 供设置框自动填入（仅本机 localhost，明文仅回显给请求者本人）。
    预设商按 provider、自定义按 api_url 一一对应。"""
    provider = str(request.query.get("provider") or "")
    api_url = str(request.query.get("api_url") or "")
    return web.json_response({"provider": provider, "api_url": api_url, "key": get_optimizer_key(provider, api_url)})


async def save_prompt_optimizer_config(request):
    data = await request.json()
    # api_key 只落盘本机 keys.json（预设商按 provider、自定义按 api_url 一一对应，
    # 空串即删除该商条目）；响应永远不回显明文
    warning = ""
    if isinstance(data, dict) and "api_key" in data:
        provider = str(data.get("provider") or "")
        api_url = str(data.get("api_url") or "")
        if not set_optimizer_key(provider, api_url, data.get("api_key")):
            warning = "api_key 保存失败（磁盘写入错误）"
    response = _public_config(_normalize_config(data))
    if warning:
...
def _prepare_prompt_optimization(payload: dict) -> tuple[dict, threading.Event]:
    ...
    config = _inject_local_keys(config)
    if config.get("mode") == "local":
        model_path = _local_model_path(config)
        if model_path.suffix.lower() == ".gguf":
    ...
    # v3 适配：按提示词标签收集被引用素材、自动判定任务、按素材类型注入音频模式
    payload["media"] = _filter_media_by_prompt(payload.get("prompt") or "", payload.get("media") or [])
    payload["task"] = _auto_task(payload.get("prompt") or "", payload["media"])
    if any(str(item.get("kind") or "").lower() == "audio" for item in payload.get("media") or []):
...
def register_prompt_optimizer_routes() -> bool:
    ...
    routes.get("/h3mediaboard/optimizer/config")(get_prompt_optimizer_config)
    routes.get("/h3mediaboard/optimizer/keys")(get_prompt_optimizer_keys)
    routes.get("/h3mediaboard/optimizer/models")(list_prompt_optimizer_models)
    routes.get("/h3mediaboard/optimizer/runninghub-models")(refresh_runninghub_models)
    routes.post("/h3mediaboard/optimizer/config")(save_prompt_optimizer_config)
    routes.post("/h3mediaboard/optimizer/optimize")(optimize_prompt)
    routes.post("/h3mediaboard/optimizer/start")(start_prompt_optimization)
    routes.get("/h3mediaboard/optimizer/status")(prompt_optimization_status)
    routes.post("/h3mediaboard/optimizer/cancel")(cancel_prompt_optimization)
